...
- **Input Volume:** Identify the target raw CT dataset (`.tif` file).
...
- **Action:** Iterate through each threshold value and execute the MCP tool `segment_ct_dataset_tif()`.
...
2. Save each resulting mask into a distinct file using a standardized naming convention:
   * `segmented_mask_thresh_0.3.tif`
   * `segmented_mask_thresh_0.5.tif`
   * `segmented_mask_thresh_0.7.tif`
...
- Verify that the target raw volume `.tif` file exists and is readable before initiating threshold loops.
